...
clientInstance.on('interactionModalCreate', async (modal) => {
    await waitForMap(modal.id, clientInstance); // Pass clientInstance here
    ...
    clientInstance.names.delete(modal.id);
    clientInstance.timers.delete(modal.id);
});
...
function waitForMap(sex, clientInstance) {
